...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        oldDistribution = self.beliefs
        for old_position, prob_old in self.beliefs.items():
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, old_position))
            for new_position, prob_new in newPosDist.items():
                allPossible[new_position] += prob_old*prob_new
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = list()
        len_pos = len(self.legalPositions)
        for i in xrange(self.numParticles):
            self.particles.append(self.legalPositions[i % len_pos])
    ...
    def elapseTime(self, gameState):
        ...
        newParticles = []
        for particle in self.particles:
            newParticles.append(self.getPositionDistribution(self.setGhostPosition(gameState, particle)))
        self.particles = [util.sample(particle) for particle in newParticles]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        list_of_permutations = [item for item in permutations]
        len_pos = len(list_of_permutations)
        for i in xrange(self.numParticles):
            self.particles.append(list_of_permutations[i % len_pos])
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] != None:
                for particle in beliefDis:
                    ...
                    trueDistance = util.manhattanDistance(ghost_pos, pacmanPosition);
                    beliefDis[particle] *= emissionModels[i][trueDistance]
        for i in range(self.numGhosts):
            if noisyDistances[i] == None:
                new_dis = util.Counter()
                for particle in beliefDis:
                    new_dis[self.getParticleWithGhostInJail(particle, i)] += beliefDis[particle]
                beliefDis = new_dis
